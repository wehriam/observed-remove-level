...
class ObservedRemoveMap extends _events.default {
  constructor(db, entries, options = {}) {
    ...
    this.processQueue = new _pQueue.default({
      concurrency: 1
    });
    this.dequeueQueue = new _pQueue.default();
  }
  ...
  async set(key, value, id = (0, _generateId.default)()) {
  ...
  async shutdown() {
    clearTimeout(this.publishTimeout); // $FlowFixMe

    await new Promise(resolve => queueMicrotask(resolve));
    await Promise.all([this.processQueue.onIdle(), this.dequeueQueue.onIdle()]);
  }
